  ...
  configureServer(server) {
    server.middlewares.use('/api/config', (req, res) => {
      try {
        ...
        const bgArrayMatch = luaContent.match(/Config\.Backgrounds\s*=\s*{([\s\S]*?)}/);
        if (bgArrayMatch) {
          const fileMatch = bgArrayMatch[1].match(/file\s*=\s*'([^']+)'/);
          if (fileMatch) config.Backgrounds[0].file = fileMatch[1];
          
          const audioLinkMatch = bgArrayMatch[1].match(/audioLink\s*=\s*'([^']+)'/);
          if (audioLinkMatch) config.Backgrounds[0].audioLink = audioLinkMatch[1];

          const musicNameMatch = bgArrayMatch[1].match(/musicName\s*=\s*'([^']+)'/);
          if (musicNameMatch) config.Backgrounds[0].musicName = musicNameMatch[1];

          const musicAuthorMatch = bgArrayMatch[1].match(/musicAuthor\s*=\s*'([^']+)'/);
          if (musicAuthorMatch) config.Backgrounds[0].musicAuthor = musicAuthorMatch[1];
        }
